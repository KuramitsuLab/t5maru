    ...
    with open(working_file, mode) as w:
        with zopen(jsonl_file) as f:
            for line in f.readlines():
                d = json.loads(line)
                line = json.dumps({
                    'in': f'{prompt}{d[source]}','out': f'{d[target]}'}, ensure_ascii=False)
                print(line, file=w)


def prepare_DataFrame(df, source=0, target=1, lines=None, working_file='temp.jsonl', mode='w'):
    if lines is not None: # in-memory mode
    ...
    with open(working_file, mode) as w:
        for i in range(len(df)):
            line = json.dumps({
                'in': f'{df.iloc[i][source]}','out': f'{df.iloc[i][target]}'}, ensure_ascii=False)
            print(line, file=w)
...
def prepare_working_file(data_sources, lines=None, prefix='temp', split='train', shuffle=True, downsizing=None):
    working_file=f'{prefix}_{split}.jsonl'
    mode='w'
    for data_source in data_sources:
        if isinstance(data_source, pd.DataFrame):
            prepare_DataFrame(data_source, lines=lines, working_file=working_file, mode=mode)
        elif isinstance(data_source, str):
            if data_source.endswith('.jsonl') or data_source.endswith('.jsonl.gz'):
                prepare_jsonl(data_source, lines=lines, working_file=working_file, mode=mode)
...
def prepare_train_file(data_sources, prefix=None, pretrain=False, shuffle=True, downsizing=None):
    if not isinstance(data_sources, (list, tuple)):
        data_sources = [data_sources]
    lines = [] if prefix is None else None        
    return prepare_working_file(data_sources, lines=lines, prefix=prefix, split='train', downsizing=downsizing)
...
def prepare_valid_file(data_sources, train_file, prefix=None, pretrain=False, downsizing=None):
    if not isinstance(data_sources, (list, tuple)):
        data_sources = [data_sources]
    valids = []
    for data_source in data_sources:
    ...
    lines = [] if prefix is None else None        
    if len(valids) > 0:
        valid_file, valid_len = prepare_working_file(valids, lines=lines, prefix=prefix, split='valid', shuffle=False, downsizing=downsizing)
        return valid_file
    else:
